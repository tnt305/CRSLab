...
class Config:
    """Configurator module that loads the defined parameters."""
    ...
    def set_model_name(self):
        ...
        tokenize = ', '.join(tokenize.values()) if isinstance(tokenize, dict) else tokenize

        model = self.opt.get('model')
        rec_model = self.opt.get('rec_model')
        conv_model = self.opt.get('conv_model')
        policy_model = self.opt.get('policy_model')

        model_name = model if model else '_'.join(filter(None, [rec_model, conv_model, policy_model]))
    ...
    def setup_logging(self, debug):
        log_name = self.opt.get("log_name", f"{self.opt['dataset']}_{self.opt['model_name']}_{time.strftime('%Y-%m-%d-%H-%M-%S')}.log")
    ...
    def __getitem__(self, item):
        return self.opt.get(item)

    def get(self, item, default=None):
        return self.opt.get(item, default)
